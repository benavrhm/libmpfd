/**
   ...
   Code refactored 2014 by Jonathan Ben-Avraham <yba@tkos.co.il>
...
void MPFD::Parser::SetContentType(const std::string type)
{
	if (type.find("multipart/form-data;") != 0) {
		throw MPFD::Exception(std::string("Content type is not "
			"multipart/form-data\"\nIt is \"") +
			type + std::string("\""));
	}
	...
	if (bp == std::string::npos)
		throw MPFD::Exception(std::string("Cannot find boundary in "
			"Content-type: \"") + type + std::string("\""));
...
void MPFD::Parser::AcceptSomeData(const char *data, const long length)
{
	...
	if (DataCollector == NULL) {
		...
		DataCollector = (char*) realloc(DataCollector,
			DataCollectorLength + length);
...
bool MPFD::Parser::ProcessContentOfTheField()
{
	...
	if (BoundaryPosition >= 0) {
		...
		DataLengthToSendToField =
			DataCollectorLength - (Boundary.length() + 2);
	}
...
printf(":::WaitForHeadersEndAndParseThem\n");
	for (int i = 0; i < DataCollectorLength - 3; i++)
	{
		if ((DataCollector[i] == 13) &&
			(DataCollector[i + 1] == 10) &&
			(DataCollector[i + 2] == 13) &&
...
/**
 */
std::string MPFD::Parser::GetTempDirForFileUpload() {
...
printf("::::_ParseHeaders\n");
	...
	if (headers.find("Content-Disposition: form-data;") == std::string::npos)
		throw Exception(std::string("Accepted headers of field does "
			"not contain \"Content-Disposition: "
			"form-data;\"\nThe headers are: \"") +
			headers + std::string("\""));
	...
	if (name_end_pos == std::string::npos)
		throw Exception(std::string("Cannot find closing quote of "
...
printf("::::filename is [%s]\n", filename.c_str());
	...
	if (content_type_pos != std::string::npos)
	{
		...
		for (int i = content_type_pos + 14;
			(i < headers.length()) && (!content_type_end_pos);
			i++)
		{
